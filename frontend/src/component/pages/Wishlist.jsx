    ...
    return ( 
        ...
        <div  key={product._id} className='grid md:flex justify-center py-4 mb-3 bg-[#f7f2ef]'>
            <Link to={`/product/${product._id}`}>
                  <img src={product.img} alt={product.title} className="w-32 h-32 lg:h-48 lg:w-48 object-cover" />
                  </Link>
                  <div className="mx-4 py-1">
                  <Link to={`/product/${product._id}`}>
                    <p className="font-semibold text-start text:sm md:text-base">{product.title}</p>
