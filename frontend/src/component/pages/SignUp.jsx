...
const SignUp = () => {
  ...
  const handleSubmit = async e => {
    ...
    await register(dispatch, { username: username.trim(), email: email.trim(), password });
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          <motion.div variants={STAGGER} transition={{ duration: 0.45, ease: [0.23, 1, 0.32, 1] }}>
            <label
              ...
              style={{ fontSize: '0.8125rem', color: 'var(--rw-muted)', fontFamily: "'Outfit', sans-serif", lineHeight: 1.5 }}
            >
              <input
                type="checkbox"
                checked={termsAccepted}
                onChange={e => setTermsAccepted(e.target.checked)}
                style={{ accentColor: 'var(--rw-gold)', width: '14px', height: '14px', marginTop: '2px', flexShrink: 0 }}
              />
              <span>
                I agree to the{' '}
                <Link
                  to="/terms"
                  style={{ color: 'var(--rw-gold)', textDecoration: 'none', fontWeight: 500 }}
                >
                  terms &amp; conditions
                </Link>
              </span>
